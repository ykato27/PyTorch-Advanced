...
def main(
    ...
    if num_jobs == 1:
        ...
        status_lst = Parallel(n_jobs=num_jobs)(
            delayed(download_clip_wrapper)(row, label_to_dir, trim_format, tmp_dir)
            for i, row in dataset.iterrows()
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str,
        help=(
            "CSV file containing the following format: "
            "YouTube Identifier,Start time,End time,Class label"
        ),
    )
    p.add_argument(
        "output_dir", type=str, help="Output directory where videos will be saved."
    )
    p.add_argument(
        "-f",
        "--trim-format",
