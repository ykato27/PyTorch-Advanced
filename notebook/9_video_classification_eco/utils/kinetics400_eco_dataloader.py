...
class Stack:
    ...
    def __call__(self, img_group):
        ...
        ret = torch.cat(
            [(x.flip(dims=[0])).unsqueeze(dim=0) for x in img_group], dim=0
        )  # frames次元で結合
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def pull_item(self, index):
        ...
        img_group = self._load_imgs(dir_path, self.img_tmpl, indices)  # リストに読み込む
    ...
    def _get_indices(self, dir_path):
        ...
        indices = (
            np.array([int(tick / 2.0 + tick * x) for x in range(self.num_segments)]) + 1
        )
